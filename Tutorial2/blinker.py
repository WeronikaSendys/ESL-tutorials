...
@chunk
def blinker_hierarchical(clk_i, led_o, length):
    '''
    Inputs:
...
blinker_hierarchical(clk, led, 3)         # Instantiate a three-bit blinker and attach I/O signals.
clk_sim(clk, num_cycles=16)  # Apply 16 clock pulses.
show_text_table()             # Look at the waveforms.

toVerilog(blinker_hierarchical, clk_i=clk, led_o=led, length=22)
